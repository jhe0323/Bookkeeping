...
def convert_fraction_to_dominant_onehot(
    data: np.ndarray,
    *,
    keep_zero_cells: bool = True,
) -> np.ndarray:
    """
    Convert fraction PFT map to dominant-PFT one-hot map.

    Parameters
    ----------
    data : np.ndarray
        Shape (T, C, Y, X), fraction map.
    keep_zero_cells : bool
        If True, cells with sum<=0 remain all-zero.
        If False, such cells are assigned to class 1 by default.

    Returns
    -------
    np.ndarray
        Shape (T, C, Y, X), one-hot dominant map.
    """
    if data.ndim != 4:
        raise ValueError(f"Expected 4D array (T,C,Y,X), got {data.shape}")

    T, C, Y, X = data.shape
    out = np.zeros_like(data, dtype=np.float32)

    total = np.nansum(data, axis=1)  # (T, Y, X)
    dominant = np.nanargmax(data, axis=1)  # (T, Y, X), 0-based class index
...
def main() -> None:
    ...
    parser.add_argument(
        "--dominant",
        action="store_true",
        help="If set, convert 1-degree fraction map to dominant-PFT one-hot map.",
    )
    parser.add_argument(
        "--keep-zero-cells",
        action="store_true",
        help="If set with --dominant, cells with total fraction <= 0 remain all-zero. "
             "Otherwise they are assigned to class 1.",
    )
    ...
    data_out = luce_1deg
    if args.dominant:
        data_out = convert_fraction_to_dominant_onehot(
            luce_1deg,
            keep_zero_cells=args.keep_zero_cells,
        )
        sum_dom = np.nansum(data_out, axis=1)
        print("Dominant 1° map shape:", data_out.shape)
        print("Dominant 1° global mean sum over veget:", float(np.nanmean(sum_dom)))
        if np.any(sum_dom > 0):
            print("Dominant 1° land-only mean sum over veget:",
                  float(np.nanmean(sum_dom[sum_dom > 0])))
            print("Dominant 1° land-only min/max sum over veget:",
                  float(np.nanmin(sum_dom[sum_dom > 0])),
                  float(np.nanmax(sum_dom[sum_dom > 0])))
