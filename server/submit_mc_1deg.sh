...
# Usage:
#   bash server/submit_mc_1deg.sh START_SAMPLE END_SAMPLE [START_BAND] [END_BAND]
#
# Re-submission is safe: current completed bands are skipped by fingerprint checks.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEMPLATE="${SCRIPT_DIR}/run_mc_1deg.sh"
GENERATED="${SCRIPT_DIR}/generated_mc_1deg"
...
if (( START_BAND < 0 || END_BAND > 35 || START_BAND > END_BAND )); then
    echo "Invalid band range; allowed 0-35" >&2
    exit 2
fi
...
for SAMPLE_ID in $(seq "${START_SAMPLE}" "${END_SAMPLE}"); do
    ...
    for BAND_ID in $(seq "${START_BAND}" "${END_BAND}"); do
        ...
        JOB_SCRIPT="${GENERATED}/run_mc_1deg_s${SAMPLE_PAD}_b${BAND_PAD}.sh"
